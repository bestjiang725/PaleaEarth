import { Layout, Typography, Card, Table, Divider } from 'antd'
...
export default function DocsPage() {
  return (
    <Layout style={{ height: '100vh' }}>
      <Header />
      <Layout.Content style={{ padding: '24px 48px', overflow: 'auto' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <Title level={2}>古地球气候剖面可视化系统 — 使用文档</Title>

          <Title level={3}>概述</Title>
          <Paragraph>
            本系统提供古地球气候模拟数据的交互式可视化，覆盖 430-505 Ma（百万年前）古生代（Paleozoic Era）的寒武纪、奥陶纪和志留纪。
            数据来源自 UK Met Office 统一模型（Unified Model v4.5/4.6）的古气候模拟输出。
          </Paragraph>

          <Title level={3}>使用方法</Title>
          <Card size="small" style={{ marginBottom: 16 }}>
            <Paragraph>
              <Text strong>1.</Text> 在左侧面板选择地质年代（通过纪选择器或滑动条）
            </Paragraph>
            <Paragraph>
              <Text strong>2.</Text> 在气候变量面板选择要显示的变量（如地表温度、降水等）
            </Paragraph>
            <Paragraph>
              <Text strong>3.</Text> 地图区域将自动生成并显示气候数据覆盖图
            </Paragraph>
            <Paragraph>
              <Text strong>4.</Text> 点击地图上的任意位置查看该点的具体数值
            </Paragraph>
            <Paragraph>
              <Text strong>5.</Text> 在底部数据面板查看不同地点的气候时间序列
            </Paragraph>
          </Card>

          <Title level={3}>数据规范</Title>
          <Table
            dataSource={[
              ...
              { key: 'ages', label: '地质年代', value: '16个时间切片 (430-505 Ma)' },
            ]}
            columns={[
          ...
          <Paragraph type="secondary">
            PaleoEarth Visualizer v1.0.0 · Phase 1 MVP · 古地球气候剖面可视化系统
          </Paragraph>
        </div>
      </Layout.Content>
    </Layout>
