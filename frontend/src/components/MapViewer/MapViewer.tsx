...
export default function MapViewer() {
  ...
  const handleZoomOut = () => {
    setZoom((z) => {
      if (z <= 1.5) { setPanX(0); setPanY(0); return 1 }
      return z - 0.5
    })
  }
  ...
  return (
    <div style={{ flex: 1, position: 'relative', overflow: 'hidden', minHeight: 0, background: '#080b10' }}>
      {/* Status */}
      <div style={{ position: 'absolute', top: 14, left: 16, zIndex: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        {selectedAgeMa == null && (
          <Tag style={{
            fontSize: 12, padding: '5px 14px',
            background: 'rgba(255,255,255,0.03)', border: '1px solid #192030',
      ...
      <div
        style={{
          width: '100%', height: '100%', position: 'relative', overflow: 'hidden', cursor,
        }}
      ...
      >
        ...
        {!overlayUrl && (
          <div style={{
            ...
            pointerEvents: 'none', flexDirection: 'column', gap: 16,
          }}>
            <div style={{
            ...
            <div>
              <div style={{ fontSize: 20, color: '#8899aa', fontWeight: 300, textAlign: 'center' }}>
                DeepEarth
              </div>
              <div style={{ fontSize: 12, color: '#4a5568', marginTop: 6, textAlign: 'center' }}>
                选择地质年代与气候变量以查看数据
              </div>
            </div>
          </div>
        ...
        {overlayUrl && !imgLoaded && !imgError && (
          <div style={{
            position: 'absolute', inset: 0, display: 'flex',
            alignItems: 'center', justifyContent: 'center',
            background: 'rgba(8,11,16,0.6)', pointerEvents: 'none',
          }}>
            <Spin size="large" />
          </div>
        )}
        ...
        {overlayUrl && imgError && (
          <div style={{
            ...
            justifyContent: 'center', background: 'rgba(8,11,16,0.7)',
            color: '#ef4444', fontSize: 14, pointerEvents: 'none', flexDirection: 'column', gap: 6,
          }}>
        ...
        {clickedPoint && (
          <div style={{
            ...
            background: 'rgba(13,17,25,0.94)', border: '1px solid #192030',
            color: '#e6edf3', padding: '10px 16px', borderRadius: 10,
            fontSize: 12, pointerEvents: 'none', whiteSpace: 'nowrap',
            zIndex: 20, backdropFilter: 'blur(10px)',
          }}>
            <div style={{ color: '#8899aa', marginBottom: 2 }}>
              <AimOutlined /> {clickedPoint.lon.toFixed(2)}°E, {clickedPoint.lat.toFixed(2)}°N
            </div>
            {clickedPoint.loading ? (
              <Spin size="small" />
            ) : clickedPoint.result ? (
              <div style={{ fontWeight: 600, fontSize: 15, color: '#93c5fd' }}>
      ...
      <div style={{
        ...
        border: '1px solid #192030', padding: 4,
        ...
        >
          <PlusOutlined style={{ fontSize: 14, color: '#8899aa' }} />
        </button>
        <div style={{ height: 1, background: '#192030', margin: '2px 8px' }} />
        <button onClick={handleZoomOut} style={btnStyle}
          onMouseEnter={(e) => (e.currentTarget.style.background = '#1e2a3a')}
          onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
        >
          <MinusOutlined style={{ fontSize: 14, color: '#8899aa' }} />
        </button>
        <div style={{ height: 1, background: '#192030', margin: '2px 8px' }} />
        <button onClick={handleReset} style={btnStyle}
          ...
          onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
        >
          <ExpandOutlined style={{ fontSize: 13, color: '#8899aa' }} />
        </button>
      </div>
      ...
      {zoom > 1 && (
        <div style={{
          position: 'absolute', bottom: 20, right: 64, zIndex: 10,
          color: '#8899aa', fontSize: 11, pointerEvents: 'none',
      ...
      <div style={{
        ...
        background: 'rgba(13,17,25,0.9)', borderRadius: 10,
        border: '1px solid #192030', padding: '10px 16px',
        display: 'flex', alignItems: 'center', gap: 10,
        backdropFilter: 'blur(10px)',
      }}>
        <span style={{ fontSize: 11, color: '#4a5568', letterSpacing: 0.5 }}>透明度</span>
        <input type="range" min={0} max={1} step={0.05} value={overlayOpacity}
          onChange={(e) => setOverlayOpacity(parseFloat(e.target.value))}
          style={{ width: 72, accentColor: '#4d9fff' }}
        />
        <span style={{ color: '#93c5fd', fontFamily: "'Fira Code', monospace", fontSize: 11, width: 30 }}>
